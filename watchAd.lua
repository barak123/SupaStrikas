...
function ballPack.new (jsonScale)
	...
	self.skeletonData = self.json:readSkeletonDataFile("WatchAdBtn.json")
	...
	self.stateData:setMix("Drop", "Idel", 1)
	...
	function self:open()	
		...
		self.state:setAnimationByName(0, "Idel_Down", false, 1)
	end
	...
	function self:drop()	
		...
		local lowDuration = self.skeletonData:findAnimation("Drop").duration * 1000

		timer.performWithDelay(lowDuration,function ()
