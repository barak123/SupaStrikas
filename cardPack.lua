...
local cardPack = {}
function cardPack.new (skeletonData)
	...
	self.json.scale = 0.3
	self.skeletonData = self.json:readSkeletonDataFile("Packs/Pack.json")
	...
	function self.skeleton:createImage (attachment)
		...
		local newPart =  display.newImage("Packs/" .. attachment.name .. ".png")
	...
	function self:open(packType)
		if packType ==	"rare" then
			self.skeleton:setSkin("Rare")
		elseif packType ==	"epic" then	
			self.skeleton:setSkin("Epic")
		else
			self.skeleton:setSkin("Common")
		end	
