...
local bubble = {}
function bubble.new (skeletonData)
	...
	self.skeletonData = self.json:readSkeletonDataFile("Bubble/CoachBubble.json")
	...
	function self.skeleton:createImage (attachment)
		...
		local newPart =  display.newImage("Bubble/" .. attachment.name .. ".png")
	...
	self.state:setAnimationByName(0, "GoodIn", false, 0)
	...
	--	self.state:setAnimationByName(0, "GoodIn", true, 0)
	...
	function self:bad()
		isFloor = false
	self.state:setAnimationByName(0, "BadIn", false, 0)
	end
	...
	function self:floor()
		...
		if not isFloor then
			--self.state:setAnimationByName(0, "FloorIn", false, 0)
		end
